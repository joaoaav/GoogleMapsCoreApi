...
namespace GoogleMapsCoreApi
{
	...
	public static class WebClientExtensionMethods
    {
        ...
        public static Task<byte[]> DownloadDataTaskAsync(this HttpClient client, Uri address, TimeSpan timeout)
        {
            return client.DownloadDataTaskAsync(address, timeout);
        }
        ...
        /// <returns>A Task with the future value of the downloaded string.</returns>
        /// <exception cref="ArgumentNullException">Thrown when a null value is passed to the client or address parameters.</exception>
        public static Task<byte[]> DownloadDataTaskAsync(this HttpClient client, Uri address, CancellationToken token)
        {
            return client.DownloadDataTaskAsync(address, InfiniteTimeout);
        }
        ...
        /// <exception cref="ArgumentNullException">Thrown when a null value is passed to the client or address parameters.</exception>
        /// <exception cref="ArgumentOutOfRangeException">Thrown when the value of timeout is neither a positive value or infinite.</exception>
        public static Task<byte[]> DownloadDataTaskAsync(this HttpClient client, Uri address, TimeSpan timeout, CancellationToken token)
        {
            ...
            try
            {
                client.DownloadDataTaskAsync(address, token);
            }
            catch
            {
                //client.DownloadDataCompleted -= completedHandler;
